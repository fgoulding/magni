...
export default async function ResumeWorkoutPage({ params }: { params: Promise<{ sessionId: string }> }) {
  const user = await requireUser().catch(() => redirect("/login"));
  const session = getWorkout(user.id, numberParam((await params).sessionId));
  if (!session) notFound();
  if (session.status !== "in_progress" || session.program_id === null) redirect(`/workouts/${session.id}`);
  ...
  return <div className="safe-x flex flex-col gap-4 py-5">
    <Link href={`/workouts/${session.id}`} className="touch-target inline-flex items-center text-sm font-semibold text-muted">← Workout details</Link>
