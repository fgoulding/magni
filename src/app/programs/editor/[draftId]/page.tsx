...
import { getEditorDraft } from "@/features/program-editor/repository";
...
export default async function EditorPage({ params, searchParams }: { params: Promise<{ draftId: string }>; searchParams: Promise<{ from?: string }> }) {
  ...
  return <ProgramWorkspace key={draftId} userId={user.id} draftId={draftId} initialDocument={document} initialRevision={existing?.revision ?? 0} activatedProgramId={existing?.activatedProgramId ?? null} />;
}
